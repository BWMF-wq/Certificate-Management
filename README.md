...
一套面向高校学生及个人用户的证书管理 Web 应用。它将学历学位、语言考试、职业资格、技能认证、竞赛获奖、培训结业、荣誉称号等证书集中归档，并通过证书概览、证书管理、数据分析和回收站四个主要入口统一组织。

## 已实现功能

- 学号 / 邮箱注册登录，BCrypt 密码哈希与 JWT 无状态鉴权
- 登录前明确确认证书记录与真实性责任条款及平台免责边界，禁止将记录用于虚假营销、欺骗或隐瞒
- 学生之间的数据完全隔离，个人资料维护
- 证书新增、编辑、分页、关键词搜索、证书类型 / 证书级别 / 证书归属筛选和多种排序
- 输入证书名称和颁发机构后自动匹配类型与级别，展示命中依据和置信度，并允许用户手动修正
- 删除证书后先移入回收站并保留附件，支持恢复与永久删除
- 证书类型覆盖学历学位、语言考试、职业资格、技能认证、竞赛获奖、创新创业、培训结业、荣誉称号、文体活动、社会实践、志愿服务、知识产权和其他证书
- 证书级别覆盖国际级、国家级、省级、市级、区县级、校级、行业级、机构级和其他级别；无法匹配时保守归入其他类
- 证书归属分为个人、团体和单位；有效期仅作为少数证书的可选档案字段，不提供到期提醒
- PDF、JPG、PNG、WebP 附件上传、替换、下载和删除（单文件最大 10MB）
- 证书总量、本年度新增、电子附件覆盖、颁发机构、证书类型、证书级别、归属构成和近 12 个月趋势统计
- 桌面端与移动端响应式界面；证书管理支持网格（纵向卡片）与列表（紧凑行式）两种差异化显示，移动端左对齐并保持操作文案横向可读
...
集成测试覆盖注册、未授权拦截、证书分类建议、证书创建、搜索与分类筛选、证书归属、证书级别、数据概况统计、软删除、回收站查询、恢复和永久删除流程。
...
- `GET /api/certificates/classification/suggest`
- `GET|PUT|DELETE /api/certificates/{id}`
- `GET /api/certificates/trash`
- `POST /api/certificates/{id}/restore`
- `DELETE /api/certificates/{id}/permanent`
...
│     └─ views/                     登录、注册、证书概览、证书管理、回收站和账户页面
├─ deploy/

...
import type { Certificate, CertificatePayload, Category, Level } from '@/types'
...
const form=reactive<CertificatePayload>({name:'',issuer:'',category:'SKILL',level:'UNIVERSITY',issueDate:today,expiryDate:null,credentialNo:'',credentialUrl:'',description:''})
const editing=computed(()=>Boolean(props.certificate)); const title=computed(()=>editing.value?'编辑证书档案':'收录一份新证书')
watch(()=>props.open,(open)=>{if(open){const c=props.certificate;Object.assign(form,{name:c?.name||'',issuer:c?.issuer||'',category:(c?.category||'SKILL') as Category,level:(c?.level||'UNIVERSITY') as Level,issueDate:c?.issueDate||today,expiryDate:c?.expiryDate||null,credentialNo:c?.credentialNo||'',credentialUrl:c?.credentialUrl||'',description:c?.description||''});pickedFile.value=null}})
  ...
  <Transition name="modal"><div v-if="open" class="modal-backdrop" @click.self="$emit('close')"><section class="modal" role="dialog" aria-modal="true">
    ...
    <form class="modal-body" @submit.prevent="submit">
      <div class="form-grid"><div class="field span-2"><label>证书名称 *</label><input v-model="form.name" maxlength="120" placeholder="例如：大学英语六级证书"/></div><div class="field span-2"><label>颁发机构 *</label><input v-model="form.issuer" maxlength="120" placeholder="例如：教育部教育考试院"/></div>
        <div class="field"><label>证书分类 *</label><select v-model="form.category"><option v-for="[value,label] in CATEGORY_OPTIONS" :key="value" :value="value">{{label}}</option></select></div><div class="field"><label>证书级别 *</label><select v-model="form.level"><option v-for="[value,label] in LEVEL_OPTIONS" :key="value" :value="value">{{label}}</option></select></div>
        <div class="field"><label>取得日期 *</label><input v-model="form.issueDate" type="date" :max="today"/></div><div class="field"><label>到期日期</label><input v-model="form.expiryDate" type="date" :min="form.issueDate"/><small>留空代表长期有效</small></div>
        <div class="field"><label>证书编号</label><input v-model="form.credentialNo" maxlength="100" placeholder="可选"/></div><div class="field"><label>官网验证地址</label><input v-model="form.credentialUrl" type="url" maxlength="500" placeholder="https://..."/></div>

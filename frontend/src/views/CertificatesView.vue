...
function saved(){load()}
async function confirmDelete(){if(!deleting.value)return;deletingNow.value=true;try{await certificateApi.remove(deleting.value.id);toast.success('证书已移入回收站');deleting.value=null;if(result.value?.content.length===1&&page.value>0)page.value--;await load()}catch(e){toast.error(errorMessage(e))}finally{deletingNow.value=false}}
...
<template>
  <div class="certificates-page">
    <header class="page-header"><div><span class="eyebrow">个人证书管理</span><h1 class="page-title">我的证书</h1><p class="page-subtitle">新增、查询和维护个人证书记录。</p></div><div class="header-actions"><button class="btn btn-secondary" @click="router.push({name:'trash'})"><Trash2 :size="17"/>回收站</button><button class="btn btn-primary" @click="openCreate"><Plus :size="18"/>新增证书</button></div></header>
    <section class="filter-bar panel">
      <div class="search-box"><Search :size="18"/><input v-model="keyword" placeholder="搜索证书名称、颁发机构或编号"/><button v-if="keyword" @click="keyword=''" aria-label="清空"><X :size="15"/></button></div>
      <div class="select-wrap"><Filter :size="15"/><select v-model="category"><option value="">全部证书类型</option><option v-for="[value,label] in CATEGORY_OPTIONS" :key="value" :value="value">{{label}}</option></select></div>
      <div class="select-wrap"><Medal :size="15"/><select v-model="level"><option value="">全部证书级别</option><option v-for="[value,label] in LEVEL_OPTIONS" :key="value" :value="value">{{label}}</option></select></div>
      <div class="select-wrap"><UsersRound :size="15"/><select v-model="awardType"><option value="">全部证书归属</option><option v-for="[value,label] in AWARD_TYPE_OPTIONS" :key="value" :value="value">{{label}}</option></select></div>
...
<style scoped>
.certificates-page{padding:clamp(28px,4vw,52px);max-width:1500px;margin:0 auto}.page-header{display:flex;justify-content:space-between;align-items:flex-end;margin-bottom:24px}.header-actions{display:flex;gap:8px}.eyebrow{display:block;color:var(--ink-2);font-size:10px;letter-spacing:.08em;font-weight:700;margin-bottom:8px}.filter-bar{padding:10px;display:grid;grid-template-columns:minmax(230px,1.35fr) repeat(4,minmax(120px,.55fr));gap:8px;box-shadow:none}.search-box,.select-wrap{height:40px;display:flex;align-items:center;gap:8px;padding:0 11px;border:1px solid var(--line);border-radius:7px;background:var(--paper);color:var(--ink-2)}.search-box input{flex:1;min-width:0;border:0;outline:0;background:transparent;color:var(--ink);font-size:12px}.search-box button{border:0;background:transparent;display:grid;cursor:pointer}.select-wrap select{width:100%;min-width:0;border:0;outline:0;background:transparent;color:var(--ink-2);font-size:11px;cursor:pointer}.result-meta{min-height:54px;display:flex;align-items:center;justify-content:space-between;gap:16px}.result-meta>div:first-child{display:flex;align-items:center;gap:14px}.result-meta p{margin:0;color:var(--ink-2);font-size:11px}.result-meta b{color:var(--ink);font-size:15px}.result-meta>div:first-child button{border:0;background:transparent;color:var(--red);font-size:11px;cursor:pointer}.view-switch{display:flex;padding:3px;border:1px solid var(--line);border-radius:7px;background:var(--white)}.view-switch button{height:29px;padding:0 9px;border:0;border-radius:5px;background:transparent;color:var(--ink-2);display:flex;align-items:center;gap:5px;font-size:10px;cursor:pointer}.view-switch button.active{background:var(--teal);color:#fff}.certificate-grid{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:16px}.certificate-grid.list-view{grid-template-columns:1fr;gap:10px}.card-skeleton{height:250px}.empty-panel{box-shadow:none}.pagination{display:flex;justify-content:center;gap:6px;margin-top:32px}.pagination button{min-width:36px;height:36px;border:1px solid var(--line);border-radius:7px;background:var(--white);cursor:pointer}.pagination button.active{background:var(--teal);color:#fff;border-color:var(--teal)}.pagination button:disabled{opacity:.4;cursor:not-allowed}
@media(min-width:1450px){.certificate-grid:not(.list-view){grid-template-columns:repeat(4,minmax(0,1fr))}}@media(max-width:1120px){.filter-bar{grid-template-columns:1fr 1fr}.certificate-grid:not(.list-view){grid-template-columns:repeat(2,minmax(0,1fr))}}@media(max-width:650px){.certificates-page{padding:22px 16px 48px}.page-header{align-items:flex-start;gap:14px;margin-bottom:20px}.header-actions{flex-direction:column-reverse;align-items:stretch;gap:8px}.page-header>div{min-width:0}.page-header .btn{width:auto;height:38px;padding:0 12px;flex:0 0 auto;font-size:11px}.filter-bar{grid-template-columns:1fr 1fr;gap:10px;padding:12px}.search-box,.sort{grid-column:span 2}.certificate-grid{grid-template-columns:1fr!important;gap:14px}.result-meta{align-items:flex-start;padding:12px 0 8px;min-height:48px}.result-meta>div:first-child{display:grid;gap:4px}.view-switch span{display:none}.pagination button{min-width:33px;height:33px}}
</style>

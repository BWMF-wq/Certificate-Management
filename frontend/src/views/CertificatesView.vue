...
<template>
  <div class="certificates-page">
    <header class="page-header"><div><span class="eyebrow">MY CREDENTIAL ARCHIVE</span><h1 class="display page-title">证书档案</h1><p class="page-subtitle">统一保存、快速检索，随时取用每一份成果证明。</p></div><button class="btn btn-primary" @click="openCreate"><Plus :size="18"/>新增证书</button></header>
    ...
    <section v-else class="panel empty-panel"><EmptyState :title="keyword||category||status?'没有匹配的证书':'还没有证书记录'" :description="keyword||category||status?'换个关键词或筛选条件试试。':'把第一份学习成果收进档案，让每一步成长都有迹可循。'" :action="keyword||category||status?'重置筛选':'添加第一份证书'" @action="keyword||category||status?resetFilters():openCreate()"/></section>

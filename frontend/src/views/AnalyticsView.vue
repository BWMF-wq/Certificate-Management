...
<template>
  <div class="analytics-page">
    <header class="page-header"><div><span class="eyebrow">CERTIFICATE INSIGHT</span><h1 class="page-title">证书数据分析</h1><p class="page-subtitle">从证书类型、级别、归属和取得时间了解个人证书构成。</p></div><button class="btn btn-secondary" @click="router.push({ name: 'certificates' })">管理证书<ArrowRight :size="16"/></button></header>
    ...
    <template v-else-if="data">
      <section class="summary-grid">
        <article class="summary"><span><BarChart3 :size="20"/></span><div><small>证书数据样本</small><strong>{{ data.total }}</strong><p>份个人证书档案</p></div></article>
        <article class="summary"><span><Medal :size="20"/></span><div><small>主要证书级别</small><strong class="text-value">{{ leadingLevel ? LEVEL_LABELS[leadingLevel.level] : '暂无数据' }}</strong><p v-if="leadingLevel">占全部档案 {{ percentage(leadingLevel.count) }}%</p></div></article>
        <article class="summary"><span><UsersRound :size="20"/></span><div><small>主要证书归属</small><strong class="text-value">{{ leadingAward ? AWARD_TYPE_LABELS[leadingAward.awardType] : '暂无数据' }}</strong><p v-if="leadingAward">共 {{ leadingAward.count }} 项</p></div></article>
      </section>
      ...
      <section class="analysis-grid top-grid">
        ...
        <article class="panel award-panel">
          <header><div><small>OWNERSHIP</small><h2>证书归属</h2></div><UsersRound :size="18"/></header>
          ...
          <EmptyState v-else title="暂无归属数据" description="新增证书时选择个人、团体或单位。"/>
        </article>
      </section>

      <section class="analysis-grid bottom-grid">
        <article class="panel distribution-panel">
          <header><div><small>CERTIFICATE LEVEL</small><h2>证书级别分布</h2></div><Medal :size="18"/></header>
          ...
          <EmptyState v-else title="暂无级别数据" description="证书级别数据会在这里形成分布。"/>
        </article>

        <article class="panel distribution-panel category-panel">
          <header><div><small>CERTIFICATE TYPE</small><h2>证书类型构成</h2></div><Layers3 :size="18"/></header>
          ...
          <EmptyState v-else title="暂无类型数据" description="证书类型数据会在这里形成分布。"/>
        </article>
      </section>
    </template>
  </div>
...
<style scoped>
.analytics-page{padding:clamp(28px,4vw,52px);max-width:1450px;margin:0 auto}.page-header{display:flex;justify-content:space-between;align-items:flex-end;margin-bottom:24px}.eyebrow{display:block;color:var(--ink-2);font-size:10px;letter-spacing:.08em;font-weight:700;margin-bottom:8px}
.summary-grid{display:grid;grid-template-columns:repeat(3,1fr);gap:10px}.summary{min-height:104px;padding:18px;border:1px solid var(--line);border-radius:10px;background:var(--white);display:flex;align-items:center;gap:14px}.summary>span{width:40px;height:40px;border-radius:8px;display:grid;place-items:center;color:var(--red);background:rgba(201,80,61,.09)}.summary small,.summary strong,.summary p{display:block}.summary small{color:var(--ink-2);font-size:10px}.summary strong{font-size:26px;line-height:1.15;margin-top:3px}.summary strong.text-value{font-size:16px;font-weight:700;margin-top:7px}.summary p{margin:4px 0 0;color:#83909b;font-size:9px}
...
.bar-chart{height:225px;padding-top:34px;display:flex;align-items:flex-end;gap:10px}.month-bar{height:100%;flex:1;display:flex;flex-direction:column;align-items:center;justify-content:flex-end;gap:7px}.month-bar>span{height:11px;font-size:8px;color:var(--ink-2)}.month-bar i{width:min(25px,72%);min-height:4px;border-radius:3px 3px 1px 1px;background:var(--red);opacity:.82}.month-bar:hover i{opacity:1}.month-bar small{font-size:8px;color:#89959e}
...
.distribution-list{display:grid;gap:18px;margin-top:25px}.distribution-list>div>div{display:flex;justify-content:space-between;gap:12px;font-size:11px}.distribution-list b{font-size:14px}.distribution-list b small{font-weight:400;color:var(--ink-2);font-size:8px}.distribution-list i{display:block;margin-top:7px;height:7px;border-radius:3px;background:var(--cream);overflow:hidden}.distribution-list em{display:block;height:100%;border-radius:inherit;background:var(--red)}.category-panel .distribution-list em{background:var(--teal)}
